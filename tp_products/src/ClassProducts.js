...
export default class C_handleProducts extends Component {
    constructor(){
        super()
        this.state = {
            Data : [],
    ...
    handleData =async () =>{
        const getData =   await axios.get('https://dummyjson.com/products/search?q='+ this.state.inputValue)
        this.setState({Data : getData.data.products})
    }
    onChange =(e) =>{
        ...
        return(
            <div>
                <div className='fixed top-0 left-0 right-0 bg-indigo-900 mt-auto flex justify-center py-4 space-x-5 items-center'>
                    <input type='text' placeholder="Search" onChange={this.onChange} onKeyDown={this.pressEntr} className=' px-2 py-4 w-2/5 placeholder-white bg-indigo-300 border border-gray-900 rounded-md h-8'/>
                    <button onClick={this.handleData} className='bg-indigo-600 px-7 py-1 rounded-lg text-white hover:bg-indigo-500'>search</button>
                </div>
